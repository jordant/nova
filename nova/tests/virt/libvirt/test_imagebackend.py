...
class RbdTestCase(_ImageTestCase, test.NoDBTestCase):
    ...
    def test_direct_fetch_success(self):
        image_href = 'rbd://a/b/c/d'
        location = {'url': image_href}
        meta = {'disk_format': 'raw'}
        image = self.image_class(self.INSTANCE, self.NAME)

        self.mox.StubOutWithMock(image.driver, 'supports_layering')
        image.driver.supports_layering().AndReturn(True)
        self.mox.StubOutWithMock(imagebackend.images, 'get_meta')
        imagebackend.images.get_meta('context', image_href).AndReturn(
                (meta, [location]))
        ...
        image.direct_fetch('context', image_href)
